...
class PerfilVendedorMostrar extends React.Component {
  constructor(){
    super();
      ...
      (response) => {
        this.setState({
          listaProdutosUsuarios: response.data.map((livro) => {
            return <CardProduto key={livro.id} livro={livro} isDono={false} />;
          }),
        });
      }).catch(
        this.setState({
  ...
  async pesquisarUsuario(id){
    return await this.serviceUsuario.getUsuario(id).then(
      (response) => {
...
export default withParams(PerfilVendedorMostrar);
